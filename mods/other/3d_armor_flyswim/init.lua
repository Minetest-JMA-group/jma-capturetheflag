...
minetest.register_on_joinplayer(function(player)
	...
	anim_playerdata[player:get_player_name()] = {}
end)

minetest.register_on_leaveplayer(function(player)
	anim_playerdata[player:get_player_name()] = nil
end)
...
minetest.register_globalstep(function()
	for _, player in ipairs(minetest.get_connected_players()) do
		...
		local animdata	 = anim_playerdata[player:get_player_name()]
		local stg_old_pos = animdata.old_pos
		local old_anim   = animdata.old_anim
	...
	-- Full Check Active Player
			...
			if animdata.std_under_slab == true then
			   player:set_physics_override({speed = animdata.org_phy_or})
			   animdata.std_under_slab = false
			end
			...
			if crouch_sneak and
			   ...
			   not check_fsable(nodes_down,2,"a") then
					...
					if controls_wasd then
						...
						animdata.std_under_slab = true
						animdata.org_phy_or = play_or_2.speed
	...
	-----------------------------
	-- Update player meta
		animdata.old_anim = animation
		animdata.old_pos = pos
