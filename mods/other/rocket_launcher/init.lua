...
minetest.register_tool("rocket_launcher:launcher", {
	...
	on_use = function(itemstack, user)
		...
		local last_fire = rocket_cooldowns[name] or 0
		if os.time() - last_fire < COOLDOWN_TIME then
			minetest.chat_send_player(name, "Rocket Launcher is on cooldown!")
			return itemstack
		end
		if inv:contains_item("main", "rocket_launcher:rocket") then
			...
			rocket_cooldowns[name] = os.time()
		else
			minetest.chat_send_player(name, "You have no rockets!")
		end
		return itemstack
	end
})
...
rocket.on_step = function(self, dtime, moveresult)
	...
	if self.timer >= 60 then self.object:remove() end
	...
	if self.timer > 0.2 then
		...
		for _, obj in pairs(objs) do
			...
			if prop and (obj:is_player() or prop.collide_with_objects == true) then
				if can_explode(pos, self.puncher_name, self.radius) then
					tnt.boom(pos, {radius = self.radius, puncher_name = self.puncher_name})
				end
				self.object:remove()
			end
		end
...
minetest.register_on_respawnplayer(function(player)
	rocket_cooldowns[player:get_player_name()] = 0
end)

minetest.register_on_leaveplayer(function(player)
	rocket_cooldowns[player:get_player_name()] = nil
end)
