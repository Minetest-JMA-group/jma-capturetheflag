...
local default_strength =
	tonumber(core.settings:get("volumetric_lighting_default_strength") or 0.1)

local function apply_light_strength(player)
	local new_strength =
		tonumber(ctf_settings.get(player, "volumetric_lighting:light_strength"))
...
core.register_chatcommand("light_strength", {
	...
	func = function(name, param)
		...
		local current_strength = player:get_lighting().volumetric_light.strength
			or default_strength
		...
		if param == "" then
			core.chat_send_player(
				name,
				S(
					"Your current volumetric lighting strength is: @1",
					math.floor(current_strength * 1000 + 0.5) / 1000
				)
			)
			return true
		end
		...
		if new_strength == default_strength then
			core.chat_send_player(
				name,
				S("Set strength to default value (@1).", new_strength)
			)
		else
