...
local function destroy(npos, cid, c_air, c_fire,
	...
	if not def then
		return c_air
	elseif core.get_item_group(def.name, "barrier") == 1 then
		return cid
	elseif not ignore_indestructible and core.get_item_group(def.name, "immortal") == 1 then
		return cid
...
function tnt.boom(pos, def)
	...
	core.sound_play(sound, {pos = pos, gain = 2.5, pitch = math.random(10, 20) / 10,
