...
ctf_melee.simple_register_sword("ctf_mode_classes:knight_sword", {
	...
	rightclick_func = function(itemstack, user, pointed)
		if
			ctf_settings.get(user, "ctf_mode_classes:simple_knight_activate") ~= "true"
		then
			...
			ctf_modebase.update_wear.start_update(
				...
				function(item_id)
					...
					if player then
						...
						local i, newstack =
							ctf_modebase.update_wear.find_item_by_id(pinv, item_id)
...
ctf_ranged.simple_register_gun("ctf_mode_classes:ranged_rifle", {
	...
	description = "Scout Rifle\n" .. core.colorize(
		"gold",
		"(Sneak/Run) + Rightclick to launch grenade ("
			.. RANGED_COOLDOWN_TIME
			.. "s cooldown), otherwise will toggle scope"
	),
	...
	rounds = 0,
	range = 150,
	on_use = ctf_ranged.on_hp_change_gun_use(-5),
	fire_interval = 0.8,
...
local scaling_def = {
	description = "Scaling Ladder\n" .. core.colorize(
		"gold",
		"(Infinite usage, self-removes after " .. SCALING_TIMEOUT .. "s)"
	),
...
ctf_healing.register_bandage("ctf_mode_classes:support_bandage", {
	description = string.format(
		"Bandage\nHeals teammates for 4-5 HP until target's HP is equal to %d%% of their maximum HP\n"
			.. core.colorize(
				"gold",
				"(Sneak/Run) + Rightclick to become immune to damage for %ds (%ds cooldown)"
			),
		HEAL_PERCENT * 100,
		IMMUNITY_TIME,
		IMMUNITY_COOLDOWN
	...
	rightclick_func = function(itemstack, user, pointed)
		if
			ctf_settings.get(user, "ctf_mode_classes:simple_support_activate") ~= "true"
		then
			local ctl = user:get_player_control()
			if not ctl.sneak and not ctl.aux1 then
				return
...
function classes.get_skin_overlay(player_or_class, class)
	return "^ctf_mode_classes_"
		.. (class and player_or_class or classes.get_name(player_or_class))
		.. "_overlay.png"
end
...
function classes.show_class_formspec(player)
	...
	if not cooldowns:get(player) then
		if ctf_modebase.current_mode ~= "classes" then
			return
		end
		...
		ctf_gui.show_formspec(player, "ctf_mode_classes:class_form", function(context)
			...
			local next_class = class_props[class_list[wrap_class(
				table.indexof(class_list, class) + 1
			)]].name
			local prev_class = class_props[class_list[wrap_class(
				table.indexof(class_list, class) - 1
			)]].name
			...
			return ctf_gui.list_to_formspec_str({
				...
				{
					"hypertext[0,0.2;%f,1.3;title;<bigger><center><b>Class Selection</b></center></bigger>]",
					form_x,
				},
				...
				{
					"image_button[0.1,%f;0.8,0.8;creative_prev_icon.png;prev_class;]",
					bar_h - 0.9,
				},
				{
				...
				{
					"image_button[%f,%f;0.8,0.8;creative_next_icon.png;next_class;]",
					form_x - 0.9,
					bar_h - 0.9,
				},
				{
				...
				},
				{
					...
					class_prop.physics
							and class_prop.physics.speed
							and "<img name=sprint_stamina_icon.png width=20 float=left> " .. class_prop.physics.speed .. "x Speed\n"
						or "",
				...
				{
					"button_exit[%f,%f;%f,1;select;Choose Class]",
					(form_x / 2) - (bw / 2),
					form_y,
					bw,
				},
		...
		hud_events.new(player, {
			quick = true,
			text = "You can only change your class every "
...
function classes.finish()
	for _, player in pairs(core.get_connected_players()) do
		player:set_properties({
			hp_max = core.PLAYER_MAX_HP_DEFAULT,
