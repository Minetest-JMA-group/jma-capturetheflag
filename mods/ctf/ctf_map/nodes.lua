...
ctf_map.barrier_nodes[minetest.get_content_id("ctf_map:ind_glass_red")] =
...
ctf_map.barrier_nodes[minetest.get_content_id("ctf_map:ind_water")] =
...
ctf_map.barrier_nodes[minetest.get_content_id("ctf_map:ind_lava")] =
	minetest.get_content_id("default:lava_source")
...
ctf_map.barrier_nodes[minetest.get_content_id("ctf_map:ind_stone_red")] =
...
for name, def in pairs(minetest.registered_nodes) do
	...
	if
		nodename
		and prefix
		and name ~= "default:torch"
		and not (
			def.groups and (def.groups.immortal or def.groups.not_in_creative_inventory)
		)
	then
		...
		if
			ctf_core.settings.server_mode ~= "mapedit"
...
local chest_def = {
	...
	on_metadata_inventory_put = function(pos, listname, index, stack, player)
		minetest.log(
			"action",
			string.format(
				"%s puts %s to treasure chest at %s",
				player:get_player_name(),
				stack:to_string(),
				minetest.pos_to_string(pos)
			)
	...
	on_metadata_inventory_take = function(pos, listname, index, stack, player)
		minetest.log(
			"action",
			string.format(
				"%s takes %s from treasure chest at %s",
				player:get_player_name(),
				stack:to_string(),
				minetest.pos_to_string(pos)
			)
