...
local function greet_player(player)
	core.chat_send_player(
		...
		core.colorize(ctf_map.CHAT_COLOR, "Welcome! This server is in mapedit mode.\n")
	)
				...
				func = function(pname, fields)
					...
					core.after(0.5, function()
						...
						if map then
							...
							core.chat_send_player(pname,
									core.colorize("red", "Error: Map '"..maplist_sorted[selected_map]..
...
function ctf_map.show_map_save_form(player, scroll_pos)
	...
	elements.positions = {
		...
		label = "Corners - " .. core.pos_to_string(context[player].pos1, 0) ..
	...
	for teamname, def in pairs(context[player].teams) do
		...
		elements[teamname.."_positions"] = {
			type = "button", exit = true,
			label = "Zone Bounds - " .. core.pos_to_string(context[player].teams[teamname].pos1, 0) ..
