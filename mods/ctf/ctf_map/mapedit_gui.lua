...
local function greet_player(player)
    ...
    if not core.check_player_privs(player, "ctf_map_editor") then
        ...
        core.chat_send_player(
            player:get_player_name(),
            core.colorize(ctf_map.CHAT_COLOR, 'To start, run "/ctf_map editor"')
        )
    end
end
                ...
                func = function(pname)
                    ...
                    ctf_map.get_pos_from_player(pname, 2, function(p, positions)
                        ...
                        context[p] = {
                            ...
                            license = "CC-BY",
                            others = "Other info",
                            --min_players, max_players required for a map (defaults)
                            min_players = 1,
                            max_players = 32,
                            --seasonal_start, seasonal_end required for a map (defaults)
                ...
                func = function(pname, fields)
                    core.after(0.1, function()
                        ctf_gui.old_show_formspec(pname, "ctf_map:loading", {
                            size = { x = 6, y = 4 },
                            title = "Capture The Flag Map Editor",
                            description = "Placing map '"
                            .. maplist_sorted[selected_map]
                    ...
                    core.after(0.5, function()
                        ...
                        local map =
                        ctf_map.load_map_meta(idx, maplist_sorted[selected_map])
...
function ctf_map.show_map_save_form(player, scroll_pos)
    if not context[player] then
        core.log(
            "error",
            "Cannot show save form because " .. player .. " is not editing a map!"
        )
        return
    end
    ...
    elements.positions = {
        ...
        exit = true,
        label = "Corners - "
            .. core.pos_to_string(context[player].pos1, 0)
            .. " - "
    ...
    elements.enabled = {
        ...
        label = "Map Enabled",
        pos = { 0, 2 },
        default = context[player].enabled,
        func = function(pname, fields)
            context[pname].enabled = fields.enabled == "true" or false
        end,
    }
    ...
    -- NEW: MIN/MAX PLAYERS AND SEASONAL
    elements.min_players = {
        type = "field",
        label = "Min Players for Vote",
        pos = { 0, ypos },
        size = { 4, 0.7 },
        default = context[player].min_players or "1",
        func = function(pname, fields)
        local val = tonumber(fields.min_players)
        if val and val >= 1 and math.floor(val) == val then
            context[pname].min_players = val
            else
                context[pname].min_players = 1  -- Reset to default
                ctf_map.show_map_save_form(
                    pname,
    ...
    elements.max_players = {
        type = "field",
        label = "Max Players for Vote",
        pos = { 0, ypos },
        size = { 4, 0.7 },
        default = context[player].max_players or "32",
        func = function(pname, fields)
        local val = tonumber(fields.max_players)
        if val and val >= 1 and math.floor(val) == val then
            context[pname].max_players = math.min(val, 32)  -- Cap at 32
            else
                context[pname].max_players = 32  -- Reset to default
                ctf_map.show_map_save_form(
                    pname,
                    core.explode_scrollbar_event(fields.formcontent).value
                )
    ...
    elements.seasonal_start = {
        type = "field",
        label = "Seasonal Start (MM.DD)",
        pos = { 0, ypos },
        size = { 4, 0.7 },
        default = context[player].seasonal_start or "01.01",
        func = function(pname, fields)
            local input = fields.seasonal_start:gsub("%s", "")  -- Leerzeichen entfernen
            if input == "" then
                context[pname].seasonal_start = "01.01"
                ctf_map.show_map_save_form(pname, core.explode_scrollbar_event(fields.formcontent).value)
                return
            end

            local month, day = input:match("^(%d+)%.(%d+)$")
            ...
            if month >= 1 and month <= 12 and day >= 1 and day <= 31 then
                context[pname].seasonal_start = string.format("%02d.%02d", month, day)
            else
                context[pname].seasonal_start = "01.01"
                ctf_map.show_map_save_form(pname, core.explode_scrollbar_event(fields.formcontent).value)
            end
    ...
    elements.seasonal_end = {
        type = "field",
        label = "Seasonal End (MM.DD)",
        pos = { 0, ypos },
        size = { 4, 0.7 },
        default = context[player].seasonal_end or "12.31",
        func = function(pname, fields)
            local input = fields.seasonal_end:gsub("%s", "")
            if input == "" then
                context[pname].seasonal_end = "12.31"
                ctf_map.show_map_save_form(pname, core.explode_scrollbar_event(fields.formcontent).value)
                return
            end
            ...
            if month >= 1 and month <= 12 and day >= 1 and day <= 31 then
                context[pname].seasonal_end = string.format("%02d.%02d", month, day)
            else
                context[pname].seasonal_end = "12.31"
                ctf_map.show_map_save_form(pname, core.explode_scrollbar_event(fields.formcontent).value)
            end
    ...
    elements.initial_stuff = {
        type = "field",
        label = "Map Initial Stuff",
        pos = { 0, ypos },
        size = { 6, 0.7 },
        default = table.concat(context[player].initial_stuff or { "none" }, ","),
        func = function(pname, fields)
            context[pname].initial_stuff =
            string.split(fields.initial_stuff:gsub("%s?,%s?", ","), ",")
        end,
    }
    ...
    elements.treasures = {
        ...
        label = "Map Treasures",
        pos = { 0, ypos },
        size = { ctf_gui.FORM_SIZE.x - 3.6, 2.1 },
    ...
    elements.skybox_label = {
        ...
        pos = { 0, ypos },
        label = (
            context[player].skybox_forced
            and "Skybox: Using the one provided in map folder"
        ) or "Skybox:",
    }
        ...
        elements.skybox = {
            ...
            func = function(pname, fields)
                ...
                if context[pname].skybox ~= oldval then
                    skybox.set(
                        PlayerObj(pname),
                        table.indexof(ctf_map.skyboxes, fields.skybox) - 1
                    )
                end
            end,
        }
    ...
    elements.enable_shadows = {
        type = "field",
        label = "Map Shadow intensity (0.0-1.0)",
        pos = { 0, ypos },
        size = { 4, 0.7 },
        default = context[player].enable_shadows or "0.26",
        func = function(pname, fields)
            local oldval = context[pname].enable_shadows
            context[pname].enable_shadows =
            math.max(0, math.min(1, tonumber(fields.enable_shadows or "0.26")))
            if context[pname].enable_shadows ~= oldval then
                PlayerObj(pname):set_lighting({
                    shadows = { intensity = context[pname].enable_shadows },
                })
            end
        end,
    }
    ...
    -- MAP PHYSICS
    for name, label in pairs({
        speed = "Map Movement Speed",
        jump = "Map Jump Height",
        gravity = "Map Gravity",
    }) do
        elements[name] = {
            type = "field",
            ...
            func = function(pname, fields)
                ...
                if context[pname]["phys_" .. name] ~= oldval then
                    physics.set(
                        pname,
                        "ctf_map_editor_" .. name,
                        { [name] = tonumber(fields[name] or 1) }
                    )
                end
            end,
        }
    ...
    elements.start_time = {
        type = "field",
        label = "Map start_time",
        pos = { 0, ypos },
        size = { 4, 0.7 },
        default = context[player].start_time or ctf_map.DEFAULT_START_TIME,
        func = function(pname, fields)
            local oldval = context[pname].start_time
            context[pname].start_time =
            tonumber(fields.start_time or ctf_map.DEFAULT_START_TIME)
            if context[pname].start_time ~= oldval then
                core.registered_chatcommands["time"].func(
                    pname,
                    tostring(context[pname].start_time)
                )
            end
        end,
    }
    ...
    elements.time_speed = {
        type = "field",
        label = "Map time_speed (Multiplier)",
        pos = { 0, ypos },
        size = { 4, 0.7 },
        default = context[player].time_speed or "1",
        func = function(pname, fields)
    ...
    for teamname, def in pairs(context[player].teams) do
        ...
        elements[teamname .. "_button"] = {
            type = "button",
            exit = true,
            label = "Set Flag Pos: " .. core.pos_to_string(def.flag_pos),
            ...
            func = function(pname, fields)
                ctf_map.get_pos_from_player(pname, 1, function(name, positions)
                    ...
                    local facedir = core.dir_to_facedir(
                        core.get_player_by_name(pname):get_look_dir()
                    )
                    core.set_node(
                        pos,
                        { name = "ctf_modebase:flag", param2 = facedir }
                    )
                    core.set_node(
                        vector.offset(pos, 0, 1, 0),
                        { name = "ctf_modebase:flag_top_" .. teamname, param2 = facedir }
                    )
                    core.after(0.1,
                        ctf_map.show_map_save_form,
                        pname,
        ...
        elements[teamname .. "_positions"] = {
            ...
            exit = true,
            label = "Zone Bounds - "
                .. core.pos_to_string(context[player].teams[teamname].pos1, 0)
                .. " - "
        ...
        elements[teamname .. "_look_pos"] = {
            type = "label",
            label = "Look position: "
                .. (look_pos and vector.to_string(look_pos) or "auto"),
            pos = { 0.2, idx },
    ...
    if #context[player].chests > 0 then
        for id, def in pairs(context[player].chests) do
            elements["chestzone_" .. id] = {
                type = "button",
                exit = true,
                label = "Chest Zone " .. id .. " - " .. core.pos_to_string(def.pos1, 0)
                    .. " - " .. core.pos_to_string(def.pos2, 0),
                pos = { 0, idx },
                size = { 7, ctf_gui.ELEM_SIZE.y },
                func = function(pname, fields)
                    if not context[pname].chests[id] then
                        return
                    end
    ...
    elements.cancelediting = {
        ...
        exit = true,
        pos = {
            ctf_gui.FORM_SIZE.x - ctf_gui.SCROLLBAR_WIDTH - ctf_gui.ELEM_SIZE.x - 1, idx,
        },
