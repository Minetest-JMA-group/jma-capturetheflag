	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		if pteam then
			...
			if not ctf_core.pos_inside(pointed_thing.above, ctf_teams.get_team_territory(pteam)) then
				core.chat_send_player(pname, "You can only place spikes in your own territory!")
				return itemstack
			end
...
for _, team in ipairs(ctf_teams.teamlist) do
	...
	core.register_node("ctf_map:spike_"..team, {
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			if item then
				local pname = placer:get_player_name()
				core.get_meta(pointed_thing.above):set_string("placer_team", ctf_teams.get(pname))
				core.get_meta(pointed_thing.above):set_string("placer", pname)
			end
			return item, pos
		end
...
core.register_on_player_hpchange(function(player, hp_change, reason)
	if reason.type == "node_damage" then
		...
		if reason.node_pos then
			local meta = core.get_meta(reason.node_pos)
