...
ctf_modebase.features = function(rankings, recent_rankings)
	...
	local function calc_map_multiplier()
		...
		if map_volume == nil then
			--- @type Vec3
			local size = ctf_map.current_map.size
	...
	--- @param pname string
	--- @param death_point Vec3
	--- @param teamnames string[]
	--- @param teamscores { [string]: table }
	--- @return number
	local function calc_attempt_score(pname, death_point, teamnames, teamscores)
		--- @type number
		local reward = 0.0
		for _, tname in ipairs(teamnames) do
			--- @type Vec3
			local enemy_flag = ctf_map.current_map.teams[tname].flag_pos
			--- @type number
	...
	--- @param teams_num integer
	--- @return number
	local function calc_capture_multiplier(teams_num)
		return math.exp(0.15 * (teams_num - 1))
	end

	--- @param pname string
	--- @param lost_teams string[]
	--- @param teamscores { [string]: table }
	--- @return number
	local function calc_capture_score(pname, lost_teams, teamscores)
		--- @type number
		local capture_reward = 0
		for _, lost_team in ipairs(lost_teams) do
			local score = ((teamscores[lost_team] or {}).score or 0) / 4
	...
	return {
		...
		on_flag_drop = function(player, teamnames, pteam, drop_point)
			local pname = player:get_player_name()
			...
			local teamscores = recent_rankings.teams()
			local attempt_reward =
				calc_attempt_score(pname, teamnames, teamscores, drop_point)
			recent_rankings.add(pname, { score = attempt_reward, flag_attempts = 1 })
			drop_flag(pteam)
		end,
		...
		on_flag_capture = function(player, teamnames)
			...
			local team_scores = recent_rankings.teams()
			local capture_reward = calc_capture_score(pname, teamnames, team_scores)
