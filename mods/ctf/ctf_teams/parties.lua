...
local settings_max_party_size = core.settings:get("ctf_max_party_size")
if settings_max_party_size then
	local size = tonumber(settings_max_party_size)
	if size then
		ctf_teams.MAX_PARTY_SIZE = settings_max_party_size
	end
end

local S = core.get_translator(core.get_current_modname())

local static_party_msgs = {
	party_disbanded = S("Your party has disbanded because you were the only one left"),
	inparty_cant_accept_invite = S("You are currently in a party. ")
		.. S('You must leave using "/party leave" before you can accept new invitations.'),
	party_at_team_size = S(
		"Your party is already at the team size, adding more players would make it unfair."
	),
	cant_invite_self = S("You can't invite yourself to a party, silly!"),
}
...
if tostring(core.settings:get("ctf_allow_parties")) == "true" then
	...
	partiescmdbuilder:sub("info", function(name)
		local playerPartyInfo = ctf_teams.get_player_party_info(name)
	...
	partiescmdbuilder:sub("invite :player:username", function(name, player)
		if minetest.get_player_by_name(player) then
					...
					for index, invite in ipairs(inviteInfo.outgoingInvites) do
						if invite.invited == player then
							minetest.chat_send_player(
								name,
								"You already invited " .. player .. " to your party."
							)
							return
						end
					end
			...
			local partyInfo = ctf_teams.get_player_party_info(name)
			if partyInfo then
				for index, playerName in ipairs(partyInfo.player_party) do
					if playerName == player then
						minetest.chat_send_player(
							name,
							player .. " is already in your party."
						)
						return
					end
				end
			...
			if response ~= "yes" then
				minetest.chat_send_player(
					name,
					"Could not invite " .. player .. " to your party."
				)
				if response == "over max party size" then
					minetest.chat_send_player(
						name,
						"Your party is already at the max size of "
							.. ctf_teams.MAX_PARTY_SIZE
					)
			...
			minetest.chat_send_player(
				name,
				"Inviting " .. player .. " to your party. They must accept to join."
			)

			minetest.chat_send_player(player, name .. " has invited you to their party.")
			minetest.chat_send_player(
				player,
				'Type "/party accept ' .. name .. '" to join.'
			)
	...
	partiescmdbuilder:sub("accept :player:username", function(name, player)
		if minetest.get_player_by_name(player) == nil then
			...
			if ctf_teams.get_player_party_info(name) ~= nil then
				minetest.chat_send_player(
					name,
					staticPartyMessages.inPartyCantAcceptInvite
				)
				return
			end
			...
			if response ~= "yes" then
				...
				minetest.chat_send_player(
					name,
					"Could not accept party invite from " .. player
				)
				minetest.chat_send_player(
					player,
					name .. " could not accept a party invite from you."
				)
				if response == "over max party size" then
					-- Send to person who tried to accept
					local overMaxSizeMsg = player
						.. "'s party is already at the max size of "
						.. ctf_teams.MAX_PARTY_SIZE
					minetest.chat_send_player(name, overMaxSizeMsg)
					-- Send to person who invited the player who couldn't join
					overMaxSizeMsg = "Your party is already at the max size of "
						.. ctf_teams.MAX_PARTY_SIZE
				...
				elseif response == "over team size" then
					...
					minetest.chat_send_player(
						name,
						player .. "'s party is already at the team size."
					)
					minetest.chat_send_player(
						name,
						"Adding more players would make it unfair."
					)
					-- Send to party inviter
					minetest.chat_send_player(
						player,
						"Your party is already at the team size."
					)
					minetest.chat_send_player(
						player,
						"Adding more players would make it unfair."
			...
			for inviteIndex, invite in ipairs(ctf_teams.invites) do
				if invite.inviter == player and invite.invited == name then
					local youJoinPartyMsg = "You have joined "
						.. player
						.. "'s party. This will take effect next match."
					local playerJoinYourPartyMsg = name
						.. " has joined your party. This will take effect next match"
					local inviterPartyInfo = ctf_teams.get_player_party_info(player)
			...
			minetest.chat_send_player(
				name,
				"You currently have no party invites from " .. player
			)
		end
	end)
	...
	partiescmdbuilder:sub("leave", function(name)
		local playerPartyInfo = ctf_teams.get_player_party_info(name)
		if playerPartyInfo ~= nil then
			for index, player_name in
				ipairs(ctf_teams.parties[playerPartyInfo.party_index])
			do
...
--- @param player string
function ctf_teams.get_player_party_info(player)
	if minetest.get_player_by_name(player) == nil then
		return nil
	else
...
-- Can pass either a player name or party info table from get_player_party_info
--- @param arg string | table
function ctf_teams.removeFromParty(arg)
	local playerPartyInfo = nil
	if type(arg) == "string" then
		playerPartyInfo = ctf_teams.get_player_party_info(arg)
		...
		if playerPartyLength < 2 then
			if playerPartyLength == 1 then
				for index, player_name in ipairs(playerPartyInfo.player_party) do
					minetest.chat_send_player(
						player_name,
						staticPartyMessages.partyDisbanded
					)
				end
			end
...
function ctf_teams.getPlayerInviteInfo(player)
	if minetest.get_player_by_name(player) == nil then
		...
		if
			(infoToReturn.outgoingInvites ~= nil) or (infoToReturn.incomingInvites ~= nil)
		then
			return infoToReturn
		else
			return nil
...
function ctf_teams.checkAndClearAllPartyInfo(player)
	local playerPartyInfo = ctf_teams.get_player_party_info(player)
	if playerPartyInfo ~= nil then
		ctf_teams.removeFromParty(playerPartyInfo)
	end
...
--Is not used by delete_oversized_parties which is run on round start.
function ctf_teams.canPartyAcceptNewPlayers(player)
	local player_party_info = ctf_teams.get_player_party_info(player)
	...
	local players_per_team = math.floor(#core.get_connected_players() / 2)

	if (players_per_team == 1) or (players_per_team == 0) then
		return "over team size"
	end
	if player_party_info ~= nil then
		if #player_party_info.player_party >= players_per_team then
			return "over team size"
...
function ctf_teams.delete_oversized_parties()
	...
	while removedAllOversizedParties == false do
		...
		for index, party in ipairs(ctf_teams.parties) do
			local incompleteDisbandMsg = "Your party ("
				.. #party
				.. " players) has been disbanded because "
			if #party > ctf_teams.MAX_PARTY_SIZE then
				local disbandReason = "it is over the max party size of "
					.. ctf_teams.MAX_PARTY_SIZE
				for _, player in pairs(party) do
					minetest.chat_send_player(
						player,
						incompleteDisbandMsg .. disbandReason
					)
				end
			...
			local teamSize = math.floor(
				#minetest.get_connected_players() / #ctf_teams.current_team_list
			)
			if #party > teamSize then
				local disbandReason = "it is bigger than the team size of "
					.. teamSize
					.. " players."
				for _, player in pairs(party) do
					minetest.chat_send_player(
	...
	for _, team in ipairs(ctf_teams.current_team_list) do
		...
		if team_scores[team] then
			if (team_scores[team].score or 0) >= 50 then
				...
				kd = math.max(
					kd,
					(team_scores[team].kills or bk) / (team_scores[team].deaths or bd)
				)
			end
