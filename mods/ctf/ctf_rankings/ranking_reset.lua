...
if
	mods:get_int("_reset_date") ~= 0
	and os.date("*t", mods:get_int("_reset_date")).month == os.date("*t").month
then
	...
	core.register_globalstep(function(dtime)
		...
		if timer >= CHECK_INTERVAL and not ctf_rankings.do_reset then
			...
			if
				current.year >= goal.year
				and current.month >= goal.month
				and current.day >= goal.day
			then
...
core.register_chatcommand("queue_ranking_reset", {
	...
	func = function(name, params)
		...
		if params:match("^stat") then
			...
			if date ~= 0 then
				return true,
					"Ranking reset queued for " .. os.date(
						"%a %d, %b %Y at %I:%M %p",
						date
					)
			else
