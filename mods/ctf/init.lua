...
local mods = {"classes", "classic", "nade_fight", "chaos", "rush"}
local mod_names = {"Classes", "Classic", "Nade", "Chaos", "Rush"}
...
local function generate_formspec(tab, skin_texture, target_name, league, is_online, positions, rank_data)
	...
	if tab == 1 then
		...
		local mod_idx  = tab - 1
		local mod      = mods and mods[mod_idx]
		local mod_name = mod_names and mod_names[mod_idx] or "Stats"
...
local function show_player_info(name, target_name)
	...
	ctf_gui.show_formspec(name, "player_info:" .. target_name, function(ctx)
