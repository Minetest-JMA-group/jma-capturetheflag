...
local cache = {}
local invites = {}
local quick_help = "Welcome to JMA Elysium! @1: @2\n"
...
local function get_quest_progress(name)
	...
	for _, quest in ipairs(TASKS) do
		...
		if done then
			table.insert(msg, core.colorize("#00ff00", S("[Done] @1", quest.label)))
		else
			local left = math.max(quest.goal - progress, 0)
			table.insert(
				msg,
				core.colorize("#ffcc00", S("[Required] @1: @2 left", quest.label, left))
			)
			all_done = false
		end
	end
	...
	if all_done then
		table.insert(msg, core.colorize("#00ff00", S("All tasks completed")))
	end
	...
	if not dropped then
		...
		if os.time() - last_leave > INACTIVITY_DURATION then
			...
			core.log(
				"action",
				string.format(
					"Quest progress for %s has been reset due to inactivity.",
					name
				)
			)
		end
	...
	for _, player in ipairs(core.get_connected_players()) do
		...
		if is_done and not access then
			...
			core.chat_send_player(
				...
				core.colorize(
					"#00ff00",
					"[Elysium] Quest completed! You have earned a "
						.. "ticket to Elysium for "
						.. playtime.seconds_to_clock(ACCESS_DURATION)
				)
			)
		end
	end
...
function ctf_jma_elysium.grant_access(name, duration_seconds)
	...
	if player then
		core.chat_send_player(
			...
			core.colorize(
				"#00ff00",
				"[Elysium] You have been granted access for "
...
core.register_chatcommand("elysium", {
	privs = { interact = true },
	description = S("Join Elysium (if you meet the requirements or were invited)"),
	func = function(name)
		local player = core.get_player_by_name(name)
		...
		if join_count[name] and join_count[name] >= MAX_ELYSIUM_JOINS then
			return false,
				S(
					"You cannot rejoin Elysium - maximum join attempts reached for the current match."
				)
		end
		...
		if REJOIN_COOLDOWN:get(player) then
			return false,
				S(
					"You're rejoining too quickly - please wait 1 minute before trying again."
				)
		end
			...
			join(player, name, function()
				core.chat_send_player(
					...
					S(
						quick_help,
						"[Invite]",
						"Ends after leaving Elysium or disconnecting"
					)
				)
				invites[name] = nil
			end)
		...
		if not is_done then
			return false,
				S("Complete the quest to get a ticket to access Elysium.\n") .. msg
		end
		...
		join(player, name, function()
			core.chat_send_player(
				name,
				S(quick_help, "Time left", playtime.seconds_to_clock(ACCESS_DURATION))
			)
		end)
...
core.register_chatcommand("leave", {
	privs = { interact = true },
	description = S("Leave Elysium"),
	func = function(name)
		local player = core.get_player_by_name(name)
		if not player then
			return false, S("You are offline.")
		end
		...
		if not ctf_modebase.in_game then
			return false,
				S(
					"Please wait until the game starts to leave Elysium or disconnect from the server."
				)
		end

		if ctf_jma_elysium.leave(player) then
			return true, S("You have left Elysium.")
		end

		return false,
			S("Something went wrong. Please contact Nanowolf4 (n4w@tutanota.com)")
	end,
})

core.register_chatcommand("eprogress", {
	privs = { interact = true },
	description = S("Show your Elysium quest progress"),
	func = function(name)
...
core.register_chatcommand("einvite", {
	privs = { interact = true },
	params = S("<player>"),
	description = S("Invite one player to Elysium (session-only)"),
	func = function(name, param)
		...
		if not core.get_player_by_name(target) then
			return false, S("Player must be online")
		end
		...
		if invited_count >= 1 then
			return false, S("You have already invited someone.")
		end

		if invites[target] then
			return false, S("This player has already been invited.")
		end

		local access = has_access(name)
		if not access or invites[name] then
			return false, S("You cannot invite players (no access or you were invited).")
		end
		...
		core.chat_send_player(
			...
			core.colorize(
				"#00ff00",
				S("You have received a one-time Elysium invite from @1!", name)
					.. "\n"
					.. S(
		...
		return true, S("You have invited @1 to Elysium", target)
	end,
})

core.register_chatcommand("el_ajoin", {
	privs = { ctf_admin = true },
	description = S("Join Elysium ignoring all restrictions (admin command)"),
	func = function(name)
		local player = core.get_player_by_name(name)
		if not player then
			return false, S("You are offline.")
		...
		if ctf_jma_elysium.players[name] then
			return false, S("You're already in Elysium. Use /leave to exit.")
		end
		ctf_jma_elysium.join(player)
		return true, S("You have joined Elysium.")
	end,
})
...
core.register_chatcommand("el_lock", {
	privs = { ctf_admin = true },
	params = "on|off",
	description = S("Lock or unlock Elysium"),
	func = function(name, param)
		param = param and param:lower()
		if param == "on" then
			ctf_jma_elysium.elysium_locked = true
			return true, S("Elysium is now locked for all players.")
		elseif param == "off" then
			ctf_jma_elysium.elysium_locked = false
			return true, S("Elysium is now unlocked.")
		else
			return false, S("Usage: /el_lock on|off")
		end
	end,
})
...
core.register_chatcommand("el_give_access", {
	privs = { ctf_admin = true },
	params = S("<player> <minutes>"),
	description = S("Give Elysium access to a player for a specified number of minutes"),
	func = function(name, param)
		local args = param:split(" ")
		local target = args[1]
		local minutes = tonumber(args[2])
		if not target or not minutes then
			return false, S("Usage: /el_give_access <player> <minutes>")
		end
		if minutes < 1 then
			return false, S("Invalid minutes value")
		end

		local seconds = minutes * 60
		ctf_jma_elysium.grant_access(target, seconds)
		return true,
			S("Access granted to @1 for @2", target, playtime.seconds_to_clock(seconds))
	end,
})
...
core.register_chatcommand("el_kick", {
	privs = { ctf_admin = true },
	params = "<player>",
	description = S("Kick a player from Elysium"),
	func = function(name, target)
		if not target or target == "" then
			return false, S("Usage: /el_kick <player>")
		end

		if not ctf_jma_elysium.get_player(target) then
			return false, S("This player is not in Elysium")
		end

		local player = core.get_player_by_name(target)
		if not player then
			return false, S("Player is not online but is in Elysium... how?")
		end
