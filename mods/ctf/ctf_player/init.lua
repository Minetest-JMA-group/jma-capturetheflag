...
function player_api.globalstep(dtime)
	for _, player in ipairs(core.get_connected_players()) do
		...
		if model and not player_attached[name] then
			...
			elseif
				not stab_slash_timer[name]
				or stab_slash_timer[name].state == "cooldown"
			then
				local r_hand = false
				if controls.up or controls.down or controls.left or controls.right then
					if controls.LMB or controls.RMB then
						...
						if
							not wielded or not wielded:get_definition().disable_mine_anim
						then
				...
				if r_hand then
					...
					if
						anim_data.r_hand_sleep
						or is_angle_different(anim_data.last_arm_angle, arm_angle)
					then
						player:set_bone_position(
							"Arm_Right_Rot",
							{ x = -2.1, y = 5.2, z = 0 },
							{ x = 180, y = arm_angle, z = -90 }
						)
